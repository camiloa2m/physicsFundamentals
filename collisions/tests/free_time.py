...
def run(time, n, radio):
	...
	sistema.main_loop()
	tiempos = []
	for i in range(len(sistema.free_time)):
		tiempos.append(i)
	print (datetime.now() - startTime)

	pos = sistema.lista_grande

	print("LAST FREE TIME: ")
	print(sistema.free_time[-1])

##############################################

	fig2, ax2 = plt.subplots()
	ax2.plot(tiempos, sistema.free_time, '-')

	ax2.set(xlabel='Evento', ylabel='Tiempo libre medio',
	       title='Tiempo libre medio en función del evento')
	ax2.grid()
