...
def run(time, n, radio):
	...
	sistema.main_loop()
	tiempos = []
	for i in range(len(sistema.pressures)):
		tiempos.append(i)
	...
	print("LAST PRESSURE: ")
	print(sistema.pressures[-1])
	
##############################################

	fig2, ax2 = plt.subplots()
	ax2.plot(tiempos, sistema.pressures, '-')

	ax2.set(xlabel='Evento', ylabel='Presion',
	       title='Presion en función del evento')
	ax2.grid()
